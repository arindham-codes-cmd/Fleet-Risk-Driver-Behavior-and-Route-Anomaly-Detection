...
-- top 5 risky drivers with their risk scores 
WITH stats AS (
  SELECT
    MIN(anomalies_count_daily) AS min_anom, 
    MAX(anomalies_count_daily) AS max_anom,
...
select 
 ...
 ROUND(
    10 * (0.35*beh_inv_n + 0.30*geo_n + 0.20*anom_n + 0.10*ran_n + 0.05*rdev_n)
  ,3) AS risk_score_1_10,
  RANK() OVER (ORDER BY 0.35*beh_inv_n + 0.30*geo_n + 0.20*anom_n + 0.10*ran_n + 0.05*rdev_n DESC
    ) AS risk_rank
  FROM norm
)
SELECT *
...
-- Top risky trips (simple, readable, parameterizable)
-- Simple Trip Risk (flags 0/0.33/0.67/1, severity = avg(norm_route_dev, inv_norm_behavior))
WITH base AS (
  SELECT
    trip_id,
    driver_id,
...
SELECT
    ...
    anomalies_count_daily 
      + route_anomalies_daily 
      + geofencing_violations_daily AS total_risk_events
FROM fleet_db.analytics.daily_rollup
ORDER BY day;
